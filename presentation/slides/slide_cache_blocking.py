...
class SlideCacheBlocking:
    def slide_cache_blocking(self):
        ...
        self.play(
            grupo_cols.animate.move_to(cache_box.get_center()).scale(1.05),
            run_time=1.1
        )
        ...
        desbordadas = [
            c for c in [*col0_copy, *col1_copy]
            if abs(c.get_center()[1] - cache_box.get_center()[1]) > 0.72
        ]

        texto_error = Text(
            "No caben: se relee desde RAM una y otra vez",
            font=FUENTE, font_size=18, color=NARANJA_TERRACOTA, weight=BOLD
        ).next_to(texto_lento, DOWN, buff=0.18)
        self.play(
            FadeIn(texto_error, shift=UP * 0.1),
            *[c.animate.set_fill(ROJO_CONTRA, opacity=0.9) for c in desbordadas],
        )
        ...
        self.play(FadeOut(grupo_cols, shift=LEFT * 0.6))
        self.wait(1.5)
        ...
        colores_ciclo = [NARANJA_TERRACOTA, VERDE_OLIVA]

        marcos_tiles = VGroup(*[
            SurroundingRectangle(
                VGroup(*[matriz_ram[i] for i in indices]),
                color=colores_ciclo[k % len(colores_ciclo)],
                buff=0.03, stroke_width=3.5, corner_radius=0.03,
            )
            for k, indices in enumerate(bloques_2x2)
        ])
        self.play(
            LaggedStart(*[Create(m) for m in marcos_tiles], lag_ratio=0.08),
            run_time=1.2,
        )

        for k, indices in enumerate(bloques_2x2[:3]):
            color = colores_ciclo[k % len(colores_ciclo)]
            bloque_copia = VGroup(*[matriz_ram[i] for i in indices]).copy()

            self.play(
                bloque_copia.animate.set_fill(color, opacity=0.88)
                    .move_to(cache_box.get_center()).scale(0.80),
                run_time=0.55,
            )
            self.play(
                Flash(cache_box, color=color, line_length=0.22, num_lines=7),
                FadeOut(bloque_copia, shift=RIGHT * 0.4),
                run_time=0.4,
            )
        ...
        pulsos_restantes = [
            Indicate(
                VGroup(*[matriz_ram[i] for i in indices]),
                color=colores_ciclo[k % len(colores_ciclo)], scale_factor=1.06,
            )
            for k, indices in list(enumerate(bloques_2x2))[3:]
        ]
        self.play(LaggedStart(*pulsos_restantes, lag_ratio=0.15), run_time=2.0)
